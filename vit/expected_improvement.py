...
def flops_constraint(x, embed=384, mlp_ratio=4, seq_length=197, head=6, token_layer=[4, 7, 10]):
...
def bayesian_optimisation(
    model, data_loader_train, data_loader_val, n_params, n_iters, flops_target, 
		neuron_rank, head_rank, token_rank, device, population, lb, ub, flops_mode,
    gp_params=None, random_search=False, Exploration=0.2):
	...
	for ratio in candidate_set:
		...
		set_token_selection_layer(model.module, ratio[-1], token_rank)
		...
		reset_token_selection_layer(model.module, token_rank)
	...
	for n in range(n_iters):
		...
		if random_search:
			...
			if np.random.rand() < Exploration:
				next_sample = random_point(flops_target=flops_target, population=1, lb=lb, ub=ub, n_params=n_params, flops_mode=flops_mode)
			else:
				res = minimize(lambda x: -1 * expected_improvement(x, gp_model, yp, greater_is_better=True, n_params=n_params), 
					...
					method='trust-constr',
					constraints=NonlinearConstraint(lambda x: flops_constraint(x, token_layer=token_rank), 0.9*flops_target, flops_target))
				next_sample = res.x
		...
		set_token_selection_layer(model.module, next_sample.tolist()[-1], token_rank)
		reward = gp_evaluate(data_loader_val, model, device)
	...
	x_best, y_best = BOHB(xp, yp, model, data_loader_train, data_loader_val, neuron_rank, head_rank, token_rank, device, drop_rate=0.3, max_iter=10)


	return x_best, y_best


def BOHB(xp, yp, model, data_loader_train, data_loader_val, neuron_rank, head_rank, token_rank, device, drop_rate=0.5, max_iter=10):
	...
	for n in range(max_iter):
		...
		# 计算需要保留的元素数量
		retain_count = int(len(yp) * (1-drop_rate))

		# 使用argsort函数获取排序索引，并按照yp的元素值从大到小排序
		...
		# 保留元素
		xp = xp_sorted[:retain_count]
		yp = yp_sorted[:retain_count]
